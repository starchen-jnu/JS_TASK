...
if(query!=null && query.size()>0){   
	for(var iter = query.iterator();iter.hasNext();){
		...
		if(doc !=null){
			...
			var sid=doc.getId();
			var searchParent="select * from tlk_knife_apply where ID='"+itemId+"' AND STATELABEL !='终止' ";
			...
			if(parnetquery!=null && parnetquery.size()>0){
				for(var parentiter=parnetquery.iterator();parentiter.hasNext();){
					...
					if(parentdoc!=null){
						...
						var Istatus=1;
						var insertsql="insert into tlk_blacklist (id,ITEM_USE_AREA,DOMAINID,ITEM_APPLYCO_NAME,ITEM_PASS_CODE,ITEM_APPLY_CODE,ITEM_ACCOUNT_NAME,ITEM_VALID) values('"+parentId+"','"+areas+"','"+domainId+"','"+apply_department+"','"+pass_code+"','"+apply_code+"','"+accountid+"','"+Istatus+"')";
